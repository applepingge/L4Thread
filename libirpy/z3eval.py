...
class BaseZ3IRPyEval(BaseIRPyEvaluator):
    ...
    def get_poison(self, ctx, type):
        if type.is_int():
            if type.size() == 1:
                return util.FreshBool('poison')
            else:
                return util.FreshBitVec('poison', type.size())
        if type.is_struct() or type.is_pointer():
            return dt.fresh_ptr(ctx, util.fresh_name('poison'), type)
    ...
    def shl(self, ctx, return_type, a, atype, b, btype, nuw=False, nsw=False):
        ...
        return util.partial_eval(ctx,
                util.If(z3.ULT(b, z3.BitVecVal(btype.size(), btype.size())), a << b, self.get_poison(ctx,btype)))
    ...
    def lshr(self, ctx, return_type, a, atype, b, btype, nuw=False, nsw=False):
        ...
        assert not nuw and not nsw
        return util.partial_eval(ctx,
                util.If(z3.ULT(b, z3.BitVecVal(btype.size(), btype.size())), z3.LShR(a, b), self.get_poison(ctx,btype)))
    ...
    def eq(self, ctx, return_type, a, atype, b, btype, **kwargs):
        if isinstance(a, dt.Pointer) and isinstance(b,z3.BitVecRef):
            return a == b
        if isinstance(b, dt.Pointer) and isinstance(a,z3.BitVecRef):
            return b == a
        assert atype == btype
        t = a == b
        if z3.is_bool(t):
    ...
    def ne(self, ctx, return_type, a, atype, b, btype, **kwargs):
        #assert atype == btype
        return z3.Not(self.eq(ctx, return_type, a, atype, b, btype, **kwargs))
    ...
    def constant_pointer_null(self, ctx, type):
        null_p = dt.Pointer(None, None, type=type, inited=False)
        null_p._name = z3.BitVec('null', 64)
        return null_p
...
class Z3IRPyEval(BaseZ3IRPyEval):
    ...
    def insert_value(self, ctx, return_type, undef_value, undef_type, target, target_type, **kwargs):
        assert return_type.is_struct() and target_type.is_int()
        value = dt.fresh_ptr(ctx, util.fresh_name('insert_value'), return_type)
        value.getelementptr(ctx, util.i64(0)).write(ctx, target)
        return value
    insertvalue = insert_value

    def extract_value(self, ctx, return_type, target, target_type, **kwargs):
        return target.getelementptr(ctx, util.i64(0), type=return_type).read(ctx)
    ...
    def branch(self, ctx, cond, cond_type, iftrue, iffalse):
        ...
        if ctx['depth'] >= ctx['prune_depth']:
            print "now depth >= prune_depth"
            #if Z3IRPyEval.flag:
            #    can_be_true = False
            #    can_be_false = True
            #    Z3IRPyEval.flag = False
            #else:
            #    can_be_true = True
            #    can_be_false = False
            can_be_true = not z3.is_false(
        ...
        #print 'can_be_false',can_be_false
        if can_be_false:
            try:
    ...
    newdepth = 1
    def branch2(self, ctx, cond, cond_type, iftrue, iffalse):
        self.assertion(ctx, not cond.is_poison(), "path condition is poison")
        assert itypes.integer_size(cond_type) == 1
        print "---now in func branch2----"
        print "cond: "
        print cond
        scond = util.simplify(cond)
        print "scond: "
    ...
    def undef(self, ctx, type):
        return self.get_poison(ctx, type)
