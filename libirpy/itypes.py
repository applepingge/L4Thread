...
POINTER_SIZE = 64
int_struct_name = {'i8':'', 'i16':'', 'i32':'', 'i64':''}
...
def parse_type(ctx, typ):
    ...
    if typ[0] == '{':
        ...
        typ = typ[1:-1].strip()
        assert typ[0] == 'i'
        assert int_struct_name[typ] != ''
        return ctx['types'][int_struct_name[typ]]
    if typ[0] == '%':
        if ' = type ' in typ:
...
def _get_struct_meta(ctx, struct_name):
    for k, v in ctx.metadata.items():
        if isinstance(v, tuple):
            ...
            typ, name = struct_name.split('.', 1)
            if v[0] == 'DICompositeType' and v[1].get('name') == name and v[1].get('elements') and v[1].get('size'):
                return v[1]
...
def _get_struct_fields(ctx, struct_meta):
    ...
    for element in elements:
        ...
        if 'baseType' in field_name[1]:
            ...
            if inner[0] == 'DICompositeType' and (inner[1].get('tag') == 'DW_TAG_structure_type' or inner[1].get('tag') == 'DW_TAG_class_type'):
                key = _get_struct_fields(ctx, inner[1])
        field_names.append({fname: key})
    return field_names

# no name struct problem refer found by stl
def declare_struct_type(ctx, name, *fields):
    ...
    fields = [parse_type(ctx, field) for field in fields]
    if len(fields) == 1 and fields[0].is_int():
        int_struct_name['i' + str(fields[0].size())] = '%' + name
