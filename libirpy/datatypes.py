...
class Pointer(object):
    def __init__(self, ctx, ref, geps=None, type=None, inited = True):
        ...
        self._name = None if not inited \
            else z3.BitVec((str(ref._name) if ref is not None else 'ite') + str(geps) , z3.BitVecSort(64))
        if ctx is not None and self._name is not None:
            ctx.add_assumption(self._name != z3.BitVec('null', 64))
        self._is_ite = False
        self._ite_cond = None
        self._ite_p1 = None
        self._ite_p2 = None
    ...
    def ite(self, other, cond):
        ...
        if isinstance(other,z3.z3.BitVecRef):
            p2 = Pointer(None, None, type=p1.type(), inited=False)
            p2._name = other
        ...
        p = Pointer(None, None, type=p1.type(), inited=False)
    ...
    def getelementptr_base(self, ctx, *idxs, **kwargs):
        ...
        geps = self._geps[:]
        geps.append(idxs)
        return Pointer(ctx, self._ref, geps, type=type)
        ...
        while ite_stack != []:
            ...
            if isinstance(top,Pointer):
                if top._is_ite:
                    ...
                    if not top._inited:
                        p = Pointer(None, None, type=type_arg, inited=False)
...
def fresh_ptr(ctx, name, type):
    ...
    init_ref(ctx, ref)
    return Pointer(ctx, ref, type=type)
